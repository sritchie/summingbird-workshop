...
object Serialization {
  ...
  /**
    ...
    */
  implicit val toBytes: Injection[Status, Array[Byte]] =
    Injection.connect[Status, String, Array[Byte]]
  ...
  implicit def vInjection[V: Codec]: Injection[(BatchID, V), Array[Byte]] =
    Injection.connect[(BatchID, V), (V, BatchID), Array[Byte]]

  /**
    * These injections aren't specifically used by this workshop, but
    * make it easy to write your own injections.
    */
  implicit def injectionFromBufferable[T: Bufferable]: Injection[T, Array[Byte]] =
    Bufferable.injectionOf[T]
